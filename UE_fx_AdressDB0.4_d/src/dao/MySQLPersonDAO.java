...
public class MySQLPersonDAO implements PersonDAO {
	...
	public boolean updatePerson(int id, String fieldName, String newValue) {
		String q ="UPDATE adressen SET "+fieldName+"=? WHERE id=?";// nur werte mit ?
		try (PreparedStatement updateStatement = con.prepareStatement(q)){
			updateStatement.setString(1, newValue); 
			updateStatement.setInt(2, id); 
			return 1 == updateStatement.executeUpdate();
		} catch (Exception e) { 
			e.printStackTrace();
		}
